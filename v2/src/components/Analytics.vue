...
export default {
  ...
  watch: {
    isResults: function () {
      if (this.$store.state.algo == "OFCORS (Théo Azzouza)") {
        // Mentions
        ...
        this.isResults.data.forEach((ct) => {
          if (ct.PREDICTION == 1 && loopI < 5) {
            texte2 = texte2.replace(
              ct.M1_CONTENT,
              `<span class="${colors[loopI]}">${ct.M1_CONTENT}</span>`
            );
            texte2 = texte2.replace(
              ct.M2_CONTENT,
              `<span class="${colors[loopI]}">${ct.M2_CONTENT}</span>`
            );
            loopI += 1;
          }
        });
        ...
        this.fullmanuscrit = this.isResults.data;
      }
    },
  },
